...
export default function PaymentLinkCreate() {
  ...
  const canCreateLink =
    validationStatus === "available" && linkName.length >= 3;
  ...
  const handleCreateLink = async () => {
    ...
    setIsCreating(true);
    try {
      await registerUserAlias(linkName, selectedIcon.emoji, description);
    } catch (error) {
    ...
    <Card className="bg-transparent border-none shadow-none w-full h-full p-0">
      <div className=" space-y-8">
        ...
        <Button
          ...
          className="w-full h-16 bg-green-500 hover:bg-green-600 disabled:bg-gray-300 disabled:text-gray-500 cursor-pointer text-white rounded-xl border-4 border-black font-black text-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
        >
